...
public extension UIImage {
    ...
    public class func gifImageWithURL(gifUrl:String) -> UIImage? {
        ...
        guard let imageData = try? Data(contentsOf: bundleURL) as CFData else {
    ...
    public class func gifImageWithName(name: String, bundle: Bundle?) -> UIImage? {
        guard 
            let bundleApp = bundle, let bundleURL = bundleApp.url(forResource: name, withExtension: "gif") 
            else {
    ...
    public class func delayForImageAtIndex(index: Int, source: CGImageSource!) -> Double {
        ...
        if delayObject.doubleValue == 0 { delayObject = unsafeBitCast(CFDictionaryGetValue(gifProperties, Unmanaged.passUnretained(kCGImagePropertyGIFDelayTime).toOpaque() ), to: AnyObject.self)
        }
